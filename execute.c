 ...
 */
int execute(char *cmd, char **av, char *path)
{
 ...
 */
int eway(char *cmd, char **av, char *path, char *shn)
{
	struct stat st;
	int i;
	...
	return (exec_no_path(av, path, cmd, shn));
}
 ...
 */
int exec_no_path(char **av, char *path, char *cmd, char *shn)
{
	char *where = NULL;
	...
	{
		av[0] = where;
		return (execute(cmd, av, path)); /*executing the executable*/
	}
